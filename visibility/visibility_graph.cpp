...
std::vector<float> removeLines(const std::vector<float> lines)
{
    std::vector<float> newLines = lines, newLines2;
    vec3 color;
    ...
    for(int i = 0;i < lines.size();i += 6)
    {
        ...
        for(int j = 1;j <= numberOfPoints + 3;j++)
        {
            Point newPoint;
    ...
    // Ahora vamos a hacer lo mismo pero intercambiando los puntos de inicio y fin de las líneas
    eraseCount = 0;
    newLines2 = newLines;
    for(int i = 0;i < newLines.size();i += 6)
    {
        startPoint.x = newLines[i];
        startPoint.y = newLines[i + 1];
        endPoint.x = newLines[i + 3];
        endPoint.y = newLines[i + 4];

        // theta = atan2(endPoint.y - startPoint.y, endPoint.x - startPoint.x);
        distance = sqrt(pow(endPoint.x - startPoint.x, 2) + pow(endPoint.y - startPoint.y, 2));
        
        theta = atan2(startPoint.y - endPoint.y, startPoint.x - endPoint.x);
        for(int j = 1;j <= numberOfPoints + 3;j++)
        {
            Point newPoint;
            newPoint.x = endPoint.x + j*distance*cos(theta)/numberOfPoints;
            newPoint.y = endPoint.y + j*distance*sin(theta)/numberOfPoints;
            newPoint = convertToScreen(newPoint);
            glReadPixels(newPoint.x, newPoint.y, 1, 1, GL_RGB, GL_FLOAT, &color);
            // Si el pixel en el punto medio es negro, eliminamos los puntos de inicio y fin de esta línea.
            // Pero si ese pixel está en la frontera del obstáculo, conservamos la línea.
                ...
                if(color[0] == 0.0f && color[1] == 0.0f && color[2] == 0.0f)
                {
                    ...
                    if(color[0] == 0.0f && color[1] == 0.0f && color[2] == 0.0f)
                    {
                        ...
                        if(color[0] == 0.0f && color[1] == 0.0f && color[2] == 0.0f)
                        {
                            ...
                            if(color[0] == 0.0f && color[1] == 0.0f && color[2] == 0.0f)
                            {
                                // printf("New lines size: %lu\n", newLines.size());
                                // printf("index: %d\n\n", (i - eraseCount*6 + 6));
                                newLines2.erase(newLines2.begin() + (i - eraseCount*6), newLines2.begin() + (i - eraseCount*6 + 6));
                                eraseCount++;
                                break;
                            }
                        }
