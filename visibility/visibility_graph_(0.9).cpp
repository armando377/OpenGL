...
std::vector<float> removeLines(std::vector<float> lines);
...
int main(){
    ...
    unsigned int obstaclesIndexes[obstacleIndexes_vec.size()], verticesIndexes[verticesIndexes_vec.size()], lineIndexes[] = {0, 1};
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(lineIndexes), lineIndexes, GL_STATIC_DRAW);
...
std::vector<float> removeLines(std::vector<float> lines)
{
    ...
    for(int i = 0;i < lines.size();i += 6)
    {
        ...
        Point newPoint;
        newPoint.x = startPoint.x + distance*cos(theta)/numberOfPoints;
        newPoint.y = startPoint.y + distance*sin(theta)/numberOfPoints;
